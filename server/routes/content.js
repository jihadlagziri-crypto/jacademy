...
router.post('/', requireAdmin, async (req, res) => {
  ...
  const db = await getDb();
  db.run('INSERT INTO content (niveau, matiere, type, titre, lien, duree) VALUES (?, ?, ?, ?, ?, ?)',
    [niveau, matiere, type, titre, lien || null, duree || null]
  );
  saveDb();
  const row = db.exec('SELECT * FROM content ORDER BY id DESC LIMIT 1');
...
router.put('/:id', requireAdmin, async (req, res) => {
  ...
  const db = await getDb();
  db.run('UPDATE content SET titre = ?, lien = ?, duree = ? WHERE id = ?',
...
router.put('/:id/toggle', requireAdmin, async (req, res) => {
  const db = await getDb();
  const row = db.exec('SELECT visible FROM content WHERE id = ?', [req.params.id]);
  if (row.length && row[0].values.length) {
    const current = row[0].values[0][0];
    db.run('UPDATE content SET visible = ? WHERE id = ?', [current ? 0 : 1, req.params.id]);
    saveDb();
  }
  res.json({ ok: true });
});
